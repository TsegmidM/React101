...
const DogCardInfo = ({ updateDogsInfo, dogInfo }) => {
  const [verifiedVaccines, setVerifiedVaccines] = useState(0);
  useEffect(() => {
    setVerifiedVaccines(
      dogInfo.vaccinations.reduce((acc, curr) => {
        if (curr.status === "Verified") return (acc = acc + 1);
        else return acc;
      }, 0)
    );
  }, [dogInfo]);
  return (
    <div className=" rounded flex-row w-96 h-96 bg-gray-200 m-2 justify-between">
      ...
      <Row className="flex-col">
        {[
          {
            label: `Neutered/Sprayed: ${
              dogInfo.requireNeuteredSpayed ? "Yes" : "No"
            } (Required)`,
            value: dogInfo.requireNeuteredSpayed ? (
          ...
          {
            label: dogInfo.subscription ? (
              dogInfo.subscription?.membershipName
            ) : (
              <>
                No Subsription{" "}
                {dogInfo.freeGroupId && (
                  <span className="underline text-blue-600">(Free Group)</span>
                )}{" "}
              </>
            ),
            value:
              dogInfo.subscription?.status !== "Inactive" ||
              dogInfo.freeGroupId ? (
                <FaCheckCircle color="green" />
              ) : (
                <FaTimesCircle color="red" />
          ...
          },
          {
            label: (
              <>
                ...
                {dogInfo.freeGroupId ? (
                  <span className="underline text-blue-600">(Free Group)</span>
                ) : dogInfo.subscription?.status === "Active" ? (
                  <span>Has Subsription</span>
                ) : (
                  <span>Missing</span>
                )}{" "}
              </>
              ...
              <span
                ...
                onClick={() => {
                  updateDogsInfo({
                    type: item.type,
                    data:
                      item.type === "change_subscription"
                        ? { dogId: dogInfo.id, subInfo: dogInfo.subscription }
                        : { dogId: dogInfo.id },
                  });
                }}
              >
            ...
            <Panel
              header={
                <div className="flex items-center">
                  {verifiedVaccines===dogInfo.vaccinations.length ?
                  <FaCheckCircle color="green" className="mr-2"/>
                  :
                  <FaTimesCircle color="red" className="mr-2"/>
              }
                  <span className="mr-1">Vaccinations</span>
                  <span>({verifiedVaccines}/{dogInfo.vaccinations.length})</span>
                </div>
              }
              key="1"
              className="w-80"
                  ...
                  <div className="flex  items-center">
                    <span
                      onClick={() => {
                        updateDogsInfo({
                          type: "change_vaccine",
                          data: { dogId: dogInfo.id, vacId: vac.vaccinationId },
                        });
                      }}
                    >
                      {vac.status === "Verified" ? (
