    ...
    return (
        <div className="memory-game-container" >
            <div className="section2">
                <div>
                    <h1>{!isWin ? (IsP1Next ? "Your turn" : "") : isWin === 'P1' ? "You Won" : isWin === 'Draw' ? "Draw!" : ""}</h1>
                    <h2>Player 1's score:<br></br>{score.P1}</h2>
                </div>
                    ...
                    {listOfCards.map((cell, idx) => {
                        return (
                            <div
                                ...
                                key={idx}
                                onClick={() => {
                                    Object.keys(twoFlipped).length < 2 && !turnedCards[idx] &&
                                        setTwoFlipped((currState) => ({
                                            ...currState,
                                            [idx]: { cardId: cell.id, value: cell.value, player: IsP1Next ? "P1" : "P2" }
                                        }));
                                }}
                            >
                                {turnedCards[idx] || twoFlipped[idx] ?
                                    <img src={
                                        (turnedCards[idx] ? turnedCards[idx].value : twoFlipped[idx].value)} />
                ...
                <div>
                    <h1>{!isWin ? (!IsP1Next ? "Your turn" : "") : isWin === 'P2' ? "You Won" : isWin === 'Draw' ? "Draw!" : ""}</h1>
                    <h2>Player 2's score:<br></br>{score.P2}</h2>
                </div>
            </div>
        </div>
    );
